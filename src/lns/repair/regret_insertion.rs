...
pub struct RegretInsertion<'a, const K: usize> {
    instance: &'a PDPTWInstance,
    use_empty_route: bool,
}

impl<'a, const K: usize> RegretInsertion<'a, K> {
    ...
    pub fn repair(&self, solution: &mut Solution, rng: &mut Random) {
        let mut k_insertion: Vec<(usize, Option<(Num, PDInsertion, bool)>)> = solution
            .unassigned_requests
            .iter_pickup_ids()
            .map(|pickup_id| (pickup_id, None))
            .collect(); //vec![(0, None); num_to_insert];
        let mut num_to_insert = k_insertion.len();
        ...
        for (id, (pickup_id, mut best_ins)) in k_insertion.iter_mut().enumerate() {
        ...
        while num_to_insert > 0 {
            if let Some((id, next_p_id, _, next_ins, empty_route)) = next_best_insertion {
                ...
                k_insertion.swap_remove(id);
                next_best_insertion = None;
                for (pickup_id, mut best_ins) in &mut k_insertion {
